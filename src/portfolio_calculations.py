    ...
    with open(csv_file, "r") as csvfile:
        ...
        for row in reader:
            ...
            if "(" in value and ")" in value:
                values = [
                    v.strip().replace("'", "") for v in value.strip("()").split(",")
                ]
                values = [
                    int(v)
...
def calculate_bond_equivalent_yield_from_DTB3(treasury_bill_rate_df):
    days_to_maturity = 90
    ann_ytm = (
        (treasury_bill_rate_df * days_to_maturity / 360)
        / (1 - treasury_bill_rate_df * days_to_maturity / 360)
        * 365
        / days_to_maturity
    )
    ...
    differences = (
        treasury_bill_rate_bey_df["DTB3"] - treasury_bill_rate_df["DTB3"]
    ).abs()
    if differences.mean() > 0.01:
        logger.error(
            "Average difference between BEY and original data is too large. Please inspect the results."
        )
        raise ValueError(
            "Average difference between BEY and original data is too large. Please inspect the results."
        )
...
def calculate_excess_returns_from_prices(
    portfolio_spec, stock_prices_df, treasury_bill_rate_df, log_returns
):
    ...
    average_timedelta_between_stock_prices = (
        stock_prices_df.index[1:] - stock_prices_df.index[:-1]
    ).mean()
    average_days_between_stock_prices = (
        average_timedelta_between_stock_prices.total_seconds() / 86400
    )
    if abs(average_days_between_stock_prices - calendar_days_between_rebalancing) > 1.5:
        stock_prices_df = daily_prices_to_rebalancing_frequency_and_window(
            portfolio_spec, trading_date_ts, stock_prices_df
        )
    ...
    treasury_bill_rate_bey_df = calculate_bond_equivalent_yield_from_DTB3(
        treasury_bill_rate_df
    )
    treasury_bill_rate_bey_adjusted_df = (1 + treasury_bill_rate_bey_df) ** (
        trading_days_between_rebalancing / 252
    ) - 1

    # Resample and interpolate risk-free rates to match stock returns' dates
    treasury_bill_rate_bey_resampled_df = treasury_bill_rate_bey_adjusted_df.reindex(
        stock_returns_df.index, method="ffill"
    )

    if log_returns:
        treasury_bill_rate_bey_resampled_df = np.log(
            1 + treasury_bill_rate_bey_resampled_df
        )

    # Calculate the excess returns
    stock_excess_returns_df = (
        stock_returns_df - treasury_bill_rate_bey_resampled_df.values
    )
...
def calculate_portfolio_variance(portfolio_comp_df, covariance_matrix_df):
    ...
    portfolio_variance = np.dot(
        sorted_weights_np.T, np.dot(sorted_covariance_matrix_np, sorted_weights_np)
    )
...
def calculate_rolling_window_frequency_adjusted(portfolio_spec):
    ...
    if portfolio_spec["rebalancing_frequency"] == "daily":
        ...
        rolling_window_frequency_adjusted = math.floor(
            portfolio_spec["rolling_window_days"] / 5
        )
    elif portfolio_spec["rebalancing_frequency"] == "monthly":
        rolling_window_frequency_adjusted = math.floor(
            portfolio_spec["rolling_window_days"] / 21
        )
    else:
        # Log an error and raise an exception if the rebalancing frequency is unknown
        logger.error("Unknown rebalancing frequency.")
...
def daily_prices_to_rebalancing_frequency_and_window(
    portfolio_spec, trading_date_ts, stock_prices_df
):
    ...
    rolling_window_frequency_adjusted = calculate_rolling_window_frequency_adjusted(
        portfolio_spec
    )
    ...
    position_current_date = stock_prices_df_frequency_adjusted.index.get_loc(
        trading_date_ts
    )
    ...
    stock_prices_frequency_and_window_adjusted_df = (
        stock_prices_df_frequency_adjusted.iloc[
...
def calculate_prior_w(
    ...
    if portfolio_spec["prior_weights"] == "empty":
        portfolio_comp_df = pd.DataFrame(
            {"Weight": [0] * num_stocks}, index=k_stock_prices_df.columns
        )

        portfolio_comp_df.index.name = "Stock"
    elif portfolio_spec["prior_weights"] == "value_weighted":
        portfolio_comp_df = calculate_value_weighted_portfolio(
            portfolio_spec, trading_date_ts, k_stock_market_caps_df
        )
    elif portfolio_spec["prior_weights"] == "equally_weighted":
        portfolio_comp_df = calculate_equally_weighted_portfolio(
            portfolio_spec, k_stock_prices_df
        )
    else:
...
def get_k_largest_stocks_market_caps(
    stock_market_caps_df,
    stock_prices_df,
    trading_date_ts,
    portfolio_size,
    rolling_window_days,
    ...
    tickers_list = data_handling.extract_unique_tickers(
        trading_date_ts, trading_date_ts
    )

    # Identify tickers that are present in stock_market_caps_df.columns
    present_tickers = [
        ticker for ticker in tickers_list if ticker in stock_market_caps_df.columns
    ]
    missing_fraction = (len(tickers_list) - len(present_tickers)) / len(tickers_list)
    logger.info(
        f"Fraction of tickers missing from stock_market_caps_df: {missing_fraction:.2%}"
    )
    ...
    eligible_stocks = [
        ...
        if (
            stock in stock_market_caps_df.columns
            and stock in tickers_list
            and stock_prices_df.loc[trading_date_ts, stock] is not None
            and stock_prices_df[stock]
            .loc[:trading_date_ts]
            .tail(rolling_window_days)
            .notna()
            .all()
        )
    ]
    ...
    if trading_date_ts in stock_market_caps_df.index:
        daily_market_caps = stock_market_caps_df.loc[
            trading_date_ts, eligible_stocks
        ].dropna()
        k_stock_market_caps_df = daily_market_caps.nlargest(portfolio_size)
        return k_stock_market_caps_df
    else:
        logger.error(
            f"The trading date {trading_date_ts} does not exist in the market capitalizations data."
        )
        raise ValueError(
            f"The trading date {trading_date_ts} does not exist in the market capitalizations data."
        )
...
def calculate_equally_weighted_portfolio(portfolio_spec, k_stock_prices_df):
    ...
    portfolio_comp_df = pd.DataFrame(
        {"Weight": [1 / num_stocks] * num_stocks}, index=k_stock_prices_df.columns
    )
...
def calculate_value_weighted_portfolio(
    portfolio_spec, trading_date_ts, k_stock_market_caps_df
):
    logger.info(f"Calculating market cap portfolio weights.")
    k_stock_market_caps_series = k_stock_market_caps_df.iloc[-1].sort_values(
        ascending=False
    )
...
def calculate_black_litterman_portfolio(
    ...
    k_stock_prices_frequency_and_window_adjusted_df = (
        daily_prices_to_rebalancing_frequency_and_window(
            portfolio_spec, trading_date_ts, k_stock_prices_df
        )
    )
    k_stock_excess_log_returns_frequency_and_window_adjusted = (
        calculate_excess_returns_from_prices(
            portfolio_spec,
            k_stock_prices_frequency_and_window_adjusted_df,
            treasury_bill_rate_df,
            True,
    ...
    k_stock_market_caps_latest_df = k_stock_market_caps_df.iloc[-1].sort_values(
        ascending=False
    )
    ...
    viewdict = {}
    market_prior_excess = black_litterman.market_implied_prior_returns(
        k_stock_market_caps_latest_df.squeeze(),
    ...
    bl = BlackLittermanModel(
        covariance_log_returns_df, pi=market_prior_excess, absolute_views=viewdict
    )
    ...
    bl_covariance_log_returns_with_risk_free_asset_df = (
        bl_covariance_log_returns_df.copy()
    )
    ...
    ef = EfficientFrontier(
        bl_mean_log_returns_with_risk_free_asset_series,
        bl_covariance_log_returns_with_risk_free_asset_df,
        weight_bounds=(0, 1),
    )
    raw_portfolio_comp = ef.max_quadratic_utility(
        risk_aversion=portfolio_spec["risk_aversion"]
    )

    # Convert cleaned weights to DataFrame
    portfolio_comp_df = pd.DataFrame(
        list(ef.clean_weights().items()), columns=["Stock", "Weight"]
    )
...
def calculate_log_normal_portfolio(
    portfolio_spec,
    trading_date_ts,
    k_stock_market_caps_df,
    k_stock_prices_df,
    ...
    mcmc_trace_path = os.path.join(
        mcmc_results,
        f"trace_log_normal_{scale}_{prior_weights}_{formatted_date}_{p}.nc",
    )
    mcmc_stocks_path = os.path.join(
        mcmc_results, f"stocks_log_normal_{prior_weights}_{formatted_date}_{p}.csv"
    )
    ...
    if os.path.exists(mcmc_trace_path) and stocks_mathcing:
        ...
        with pm.Model() as model:
            ...
            log_nu = pm.MvNormal(
                "log_nu", mu=pt.log(prior_w), cov=scale * pt.eye(len(prior_w))
            )
            ...
            observed_data = (
                k_stock_excess_log_returns_frequency_and_window_adjusted.values
            )
    ...
    if formatted_date == portfolio_spec["posterior_nu_plot_date"]:
        ...
        plt.suptitle(
            "Posterior Distributions of Log Portfolio Weights",
            y=0.96,
            fontweight="bold",
            fontsize=14,
            color="black",
        ...
        # Save the figure
        mcmc_posterior_path = os.path.join(
            mcmc_results,
            f"log_normal_{scale}_{prior_weights}_{formatted_date}_{p}_posterior.png",
        )
...
def calculate_jorion_hyperparameter_portfolio(
    portfolio_spec, trading_date_ts, k_stock_prices_df, treasury_bill_rate_df
):
    ...
    # Sample mean
    mu_hat_df = (
        k_stock_excess_log_returns_frequency_and_window_adjusted.mean().to_frame()
    )
    ...
    V_bar_inverse_df = pd.DataFrame(
        np.linalg.inv(V_bar_df.to_numpy()),
        index=V_bar_df.index,
        columns=V_bar_df.columns,
    )
    one_N_df = pd.DataFrame(np.ones(N), index=V_bar_inverse_df.index)
    mu_hat_g = (
        one_N_df.T.dot(V_bar_inverse_df).dot(mu_hat_df)
        / one_N_df.T.dot(V_bar_inverse_df).dot(one_N_df)
    ).values[0, 0]

    mu_hat_difference = mu_hat_df.sub(mu_hat_g * one_N_df.values, axis=0)
    lambda_hat = (N + 2) / (
        mu_hat_difference.T.dot(V_bar_inverse_df).dot(mu_hat_difference)
    ).values[0, 0]

    v_hat = (N + 2) / (
        (N + 2) + T * mu_hat_difference.T.dot(V_bar_inverse_df).dot(mu_hat_difference)
    ).values[0, 0]
    V_hat_PJ_df = (1 + 1 / (T + lambda_hat)) * V_bar_df + lambda_hat / (
        T * (T + 1 + lambda_hat)
    ) * one_N_df.dot(one_N_df.T) / (
        one_N_df.T.dot(V_bar_inverse_df).dot(one_N_df)
    ).values[
        0, 0
    ]
    ...
    ef = EfficientFrontier(
        mu_hat_PJ_with_risk_free_asset_series,
        V_hat_PJ_with_risk_free_asset_df,
        weight_bounds=(0, 1),
    )
    raw_portfolio_comp = ef.max_quadratic_utility(
        risk_aversion=portfolio_spec["risk_aversion"]
...
def calculate_shrinkage_portfolio(
    portfolio_spec, trading_date_ts, k_stock_prices_df, treasury_bill_rate_df
):
    ...
    # Mean return
    shrinkage_mean_log_returns_series = expected_returns.mean_historical_return(
        k_stock_excess_log_returns_frequency_and_window_adjusted,
        returns_data=True,
        compounding=False,
        frequency=time_periods_per_year(portfolio_spec),
    ...
    # Covariance matrix
    shrinkage_covariance_log_returns_df = risk_models.CovarianceShrinkage(
        k_stock_excess_log_returns_frequency_and_window_adjusted,
        returns_data=True,
        frequency=time_periods_per_year(portfolio_spec),
    ).ledoit_wolf()
    ...
    # Add risk free asset
    shrinkage_mean_log_returns_with_risk_free_asset_series = (
        shrinkage_mean_log_returns_series.copy()
    )
    shrinkage_mean_log_returns_with_risk_free_asset_series["RISK_FREE"] = 0

    shrinkage_covariance_log_returns_with_risk_free_asset_df = (
        shrinkage_covariance_log_returns_df.copy()
    )
...
def calculate_min_variance_portfolio(
    ...
    ef = EfficientFrontier(
        min_variance_mean_log_returns_series,
        min_variance_covariance_log_returns_df,
        weight_bounds=(0, 1),
    )
...
def calculate_portfolio_weights(trading_date_ts, portfolio_spec, market_data):
    # Unpack market data
    ...
    # Filter market caps
    k_stock_market_caps_df = stock_market_caps_df[
        k_stock_market_caps_trading_date_df.index.intersection(
            stock_market_caps_df.columns
        )
    ]
    k_stock_market_caps_df = k_stock_market_caps_df.loc[:trading_date_ts]

    # Filter stock prices
    k_stock_prices_df = stock_prices_df[
    ...
    if portfolio_spec["weights_spec"] == "value_weighted":
        ...
        portfolio_comp_df = calculate_black_litterman_portfolio(
            portfolio_spec,
            trading_date_ts,
            k_stock_market_caps_df,
            k_stock_prices_df,
        ...
        portfolio_comp_df = calculate_log_normal_portfolio(
            portfolio_spec,
            trading_date_ts,
            k_stock_market_caps_df,
            k_stock_prices_df,
        ...
        portfolio_comp_df = calculate_jorion_hyperparameter_portfolio(
            portfolio_spec, trading_date_ts, k_stock_prices_df, treasury_bill_rate_df
        )

    elif portfolio_spec["weights_spec"] == "shrinkage":
        portfolio_comp_df = calculate_shrinkage_portfolio(
            portfolio_spec, trading_date_ts, k_stock_prices_df, treasury_bill_rate_df
        )

    elif portfolio_spec["weights_spec"] == "min_variance":
        portfolio_comp_df = calculate_min_variance_portfolio(
            portfolio_spec, trading_date_ts, k_stock_prices_df, treasury_bill_rate_df
        )
...
def compute_portfolio_turnover(portfolio_comp_before_df, portfolio_comp_after_df):
    ...
    portfolio_comp_before_after_df = portfolio_comp_before_df.merge(
    ...
    portfolio_comp_before_after_df["weight_diff"] = abs(
        portfolio_comp_before_after_df["Weight_before"]
        - portfolio_comp_before_after_df["Weight_after"]
    )

    # Calculate turnover corresponding to risk free asset
    risk_free_turnover = abs(
        portfolio_comp_before_df["Weight"].sum()
        - portfolio_comp_after_df["Weight"].sum()
    )

    # Calculate total turnover
    turnover = (
        portfolio_comp_before_after_df["weight_diff"].sum() + risk_free_turnover
    ) / 2
...
class Portfolio:
    ...
    def __init__(self, ts_start_date, portfolio_spec):
        self.ts_start_date = ts_start_date
        self.portfolio_spec = portfolio_spec
        self.portfolio_simple_returns_series = pd.Series(
            dtype="float64", name=portfolio_spec["display_name"]
        )
        self.portfolio_turnover_series = pd.Series(
            dtype="float64", name=portfolio_spec["display_name"]
        )
        self.last_rebalance_date_ts = None

    def update_portfolio(self, trading_date_ts, market_data):
        # Calculate daily portfolio return
        if self.ts_start_date != trading_date_ts:
            # Filter out stocks not in the portfolio
            filtered_stock_simple_returns_series = (
                market_data["stock_simple_returns_df"]
                .loc[trading_date_ts]
                .reindex(self.portfolio_comp_df.index)
            )

            # Multiply returns by weights element-wise and then sum to get the portfolio return
            portfolio_simple_return = (
                filtered_stock_simple_returns_series * self.portfolio_comp_df["Weight"]
            ).sum()
            ...
            most_recent_treasury_bill_rate_bey = treasury_bill_rate_bey_df.asof(
                trading_date_ts
            ).iloc[0]
            risk_free_daily_return = (most_recent_treasury_bill_rate_bey + 1) ** (
                1 / 252
            ) - 1
            portfolio_simple_return += (
                1 - self.portfolio_comp_df["Weight"].sum()
            ) * risk_free_daily_return

            self.portfolio_simple_returns_series[
                trading_date_ts
            ] = portfolio_simple_return

            # Update weight for the risk-free asset
            current_risk_free_weight = 1 - self.portfolio_comp_df["Weight"].sum()
            updated_risk_free_weight = current_risk_free_weight * (
                1 + risk_free_daily_return
            )

            # Update weights for the stocks
            self.portfolio_comp_df["Weight"] = self.portfolio_comp_df["Weight"] * (
                1 + filtered_stock_simple_returns_series
            )

            # Update the total invested value by adding the updated risk-free weight
            total_value = (
                self.portfolio_comp_df["Weight"].sum() + updated_risk_free_weight
            )

            # Normalize the weights so they sum up to 1
            self.portfolio_comp_df["Weight"] = (
                self.portfolio_comp_df["Weight"] / total_value
            )
            ...
            if (
                abs(
                    (
                        self.portfolio_comp_df["Weight"].values.sum()
                        + updated_risk_free_weight / total_value
                    )
                    - 1
                )
                > 1e-5
        ...
        if self.last_rebalance_date_ts is None:
            ...
            rebalance = (
                trading_date_ts.weekday() == 2
                or (trading_date_ts - self.last_rebalance_date_ts).days > 7
            )
        ...
        if rebalance:
            ...
            self.portfolio_comp_df = calculate_portfolio_weights(
                trading_date_ts, self.portfolio_spec, market_data
            )

            if not self.last_rebalance_date_ts is None:
                turnover = compute_portfolio_turnover(
                    portfolio_comp_before_df, self.portfolio_comp_df
                )
                self.portfolio_turnover_series[trading_date_ts] = turnover
                turnover_cost = (
                    self.portfolio_spec["turnover_cost_bps"] / 10000 * turnover
                )
                self.portfolio_simple_returns_series[trading_date_ts] -= turnover_cost

            logger.info(
                f"Portfolio size {trading_date_ts}: {len(self.portfolio_comp_df.index)}"
            )
            self.last_rebalance_date_ts = trading_date_ts


def backtest_portfolio(portfolio_spec, ts_start_date, ts_end_date, market_data):
    # Trading dates
    trading_date_ts = [pd.Timestamp(ts) for ts in market_data["stock_prices_df"].index]
    trading_date_ts = [
        ts for ts in trading_date_ts if ts_start_date <= ts <= ts_end_date
    ]

    portfolio = Portfolio(trading_date_ts[0], portfolio_spec)

    for trading_date_ts in trading_date_ts:
        portfolio.update_portfolio(trading_date_ts, market_data)

    return {
        "portfolio_simple_returns_series": portfolio.get_portfolio_simple_returns_series(),
