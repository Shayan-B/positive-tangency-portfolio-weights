...
def save_stock_market_caps_to_csv(tickers, start_date, end_date):
    ...
    for index, ticker in enumerate(tickers, start=1):
        ...
        stock_market_cap_data = pd.DataFrame(
            {ticker: stock_market_caps}, index=stock_market_caps_dates
        )
        ...
        mask = (stock_market_cap_data.index >= start_date) & (
            stock_market_cap_data.index <= end_date
        )
        ...
        if not stock_market_cap_data.empty:
            ...
            stock_market_cap_data.to_csv(
                os.path.join(stock_market_caps_dir, f"{ticker}.csv")
            )
        else:
            logger.warning(
                f"No market cap data available for {ticker} in the given date range"
            )
