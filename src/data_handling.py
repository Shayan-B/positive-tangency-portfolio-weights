...
def load_all_csv_to_dataframe(directory):
    ...
    for f in files:
        ...
        df.columns = [
            f.replace(".csv", "")
        ]  # renaming the column as the file name (without the .csv extension)
...
def extract_unique_tickers(start_date_ts, end_date_ts):
    if check_directory_for_csv(sp500_components_dir):
        ...
        raise ValueError(
            f"S&P 500 historical components must be downloaded and located in /data/sp500_components. Please visit https://github.com/fja05680/sp500"
        )
    ...
    if pd.isnull(closest_start_date):
        logger.error(
            f"No S&P 500 historical components available before the specified start date: {start_date_ts}"
        )
        raise ValueError(f"No S&P 500 historical components.")

    # Filter rows between start_date and end_date
    filtered_tickers_df = tickers_df[
        (tickers_df.index >= closest_start_date) & (tickers_df.index <= end_date_ts)
    ].copy()

    # Split tickers strings into lists
    filtered_tickers_df["split_tickers"] = filtered_tickers_df[
        "S&P 500 Historical Components & Changes(08-01-2023)"
    ].str.split(",")

    # Flatten list of lists and get unique tickers
    unique_tickers = pd.unique(
        [
            ticker
            for sublist in filtered_tickers_df["split_tickers"]
...
def get_stock_prices():
    ...
    if check_directory_for_csv(stock_prices_dir):
        ...
        user_response = (
            input("Do you want to save stock prices to CSV? (Y/N): ").strip().upper()
        )

        if user_response == "Y":
            unique_tickers_list = extract_unique_tickers(
                pd.to_datetime("1999-12-31"), pd.to_datetime("2023-06-30")
            )
            alpha_vantage.save_stock_prices_to_csv(
                unique_tickers_list, str_start_date, str_end_date, max_calls_per_minute
            )
            stock_prices_df = load_all_csv_to_dataframe(stock_prices_dir)
        else:
...
def get_stock_market_caps():
    ...
    if check_directory_for_csv(stock_market_caps_dir):
        ...
        # Prompt the user for a response
        user_response = (
            input("Do you want to save market caps to CSV? (Y/N): ").strip().upper()
        )
        ...
        if user_response == "Y":
            ...
            financial_modeling_prep.save_stock_market_caps_to_csv(
                unique_tickers_list, str_start_date, str_end_date
            )
            stock_market_caps_df = load_all_csv_to_dataframe(stock_market_caps_dir)
        else:
...
def get_sp500tr_prices():
    ...
    if os.path.exists(csv_file):
        sp500tr_prices = pd.read_csv(
            csv_file, index_col=0, parse_dates=True
        )  # Assuming the first column is a date index
    else:
        # Prompt the user for a response
        user_response = (
            input("Do you want to save SP500TR data to CSV? (Y/N): ").strip().upper()
        )
...
def get_treasury_bill_rate():
    ...
    if os.path.exists(csv_file):
        ...
        treasury_bill_rate = pd.read_csv(
            csv_file, index_col=0, parse_dates=True, na_values=["."]
        )
    else:
        raise ValueError(
            f"Risk-free rate csv must be downloaded. Please visit https://fred.stlouisfed.org/series/DTB3"
        )
