...
def plot_performance(
    portfolio_setups_simple_returns_df,
...
def process_and_highlight_values(metrics_df):
    ...
    lower_is_better = {"Ann. Vol.", "Avg. Turnover"}
    ...
    for row_label, row_series in metrics_df.iterrows():
        ...
        processed_values = [
            round(100 * val, 3) if row_label not in not_percentage else round(val, 3)
        ...
        for proc_val in processed_values:
            str_val = (
                f"{proc_val:.3f}%"
                if row_label not in not_percentage
                else f"{proc_val:.3f}"
            )
...
def performance_metrics(
    ...
    portfolio_setups_metrics_df = pd.DataFrame(
        columns=portfolio_setups_simple_returns_df.columns
    )
    ...
    for column_name in portfolio_setups_simple_returns_df.columns:
        portfolio_simple_returns_series = portfolio_setups_simple_returns_df[
            column_name
        ]

        portfolio_comp_return = qs.stats.comp(portfolio_simple_returns_series)
        portfolio_setups_metrics_df.at[
            "Cum. return", column_name
        ] = portfolio_comp_return
    ...
    for column_name in portfolio_setups_excess_simple_returns_df.columns:
        portfolio_excess_simple_returns_series = (
            portfolio_setups_excess_simple_returns_df[column_name]
        )

        portfolio_sharpe_ratio = qs.stats.sharpe(portfolio_excess_simple_returns_series)
        portfolio_setups_metrics_df.at[
            "Sharpe Ratio", column_name
    ...
    for column_name in portfolio_setups_excess_simple_returns_df.columns:
        ...
        portfolio_prob_sharpe_ratio = qs.stats.probabilistic_ratio(
            portfolio_excess_simple_returns_series
        )
        portfolio_setups_metrics_df.at[
            "Prob. Sharpe Ratio", column_name
        ] = portfolio_prob_sharpe_ratio
    ...
    for column_name in portfolio_setups_excess_simple_returns_df.columns:
        ...
        portfolio_sortino_ratio = qs.stats.sortino(
            portfolio_excess_simple_returns_series
        )
        portfolio_setups_metrics_df.at[
            "Sortino Ratio", column_name
    ...
    for column_name in portfolio_setups_simple_returns_df.columns:
        ...
        portfolio_calmar_ratio = qs.stats.cagr(
            portfolio_simple_returns_series, periods=365
        ) / abs(qs.stats.max_drawdown(portfolio_simple_returns_series))
        portfolio_setups_metrics_df.at[
            "Calmar Ratio", column_name
    ...
    for column_name in portfolio_setups_turnover.columns:
        ...
        portfolio_setups_metrics_df.at[
            "Avg. Turnover", column_name
        ] = portfolio_turnover

    highlighted_metrics_df = process_and_highlight_values(portfolio_setups_metrics_df)
    highlighted_metrics_df.to_csv(
        f"../results/metrics_{portfolio_size}.csv", index=True
    )


def compute_excess_returns(portfolio_simple_returns_series, treasury_bill_rate_df):
    days_to_maturity = 90
    ann_ytm = (
    ...
    matching_treasury_bill_bey_rates_series = (
        treasury_bill_rate_bey_df["DTB3"]
        .reindex(portfolio_simple_returns_series.index)
        .ffill()
    )

    # For any NaNs at the beginning, use backfill to fill them with the next valid value
    matching_treasury_bill_bey_rates_series = (
        matching_treasury_bill_bey_rates_series.bfill()
    )
...
def full_evaluation(
    ...
    # Get excess returns
    portfolio_setups_excess_simple_returns_df = pd.DataFrame(
        columns=portfolio_setups_simple_returns_df.columns
    )
    for column_name in portfolio_setups_simple_returns_df.columns:
        portfolio_simple_returns_series = portfolio_setups_simple_returns_df[
        ...
        portfolio_excess_simple_returns_series = compute_excess_returns(
            portfolio_simple_returns_series, treasury_bill_rate_df
        )
